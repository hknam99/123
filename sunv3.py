...
# === CẤU HÌNH TELEGRAM ===
BOT_TOKEN = "7797061216:AAFGJCUZer5l_s-LR5ZYEIXLkg1Ou9C3w2k" #add bot token here
...
# Kiểm tra key còn hạn và người dùng có nhập key không trước khi gửi
def send_prediction_update(session):
    ...
    result_display = "*TÀI*" if result == "Tài" else "*XỈU*"
    prediction_display = "*TÀI*" if prediction == "Tài" else "*XỈU*"
    ...
    message = (
        ...
        f"{EMOJI['sum']} *Tổng điểm:* `{total}` | *Kết quả:* {result_display} ({'ĐÚNG' if prev_prediction_correct else 'SAI'})\n"
        f"──────────────────────────\n"
...
def background_task():
    ws_url = "wss://example.com/socket" #add websocket here
    reconnect_delay = 1
    while True:
        try:
...
def handle_telegram_updates():
    ...
    while True:
                ...
                if "callback_query" in update:
                    ...
                    if data == "help_activate":
                        help_message = (
                            ...
                            f"══════════════════════════\n"
                            f"{EMOJI['team']} Liên hệ admin: t.me/hknamvip"
                        )
                    ...
                    if text:
                        if text.startswith("/start"):
                            ...
                            buttons = [
                                ...
                                [{"text": f"{EMOJI['vip']} Thông tin cập nhật", "callback_data": "new_update"}],[{"text": f"{EMOJI['chat']} Kênh chat", "url": "https://t.me/ttt3ttts"}],[{"text": f"{EMOJI['chat']} Kênh thông báot", "url": "https://t.me/ttt3ttts1"}],[{"text": f"{EMOJI['chat']} Nhóm test bot beta", "url": "https://t.me/ttt3ttts2"}],[{"text": f"{EMOJI['money_bag']} Liên hệ mua key", "url": "https://t.me/hknamvip"}]
                            ]
        ...
        elif text.startswith("/dsadmin") and chat_id == 6020088518:
            admin_ids = get_all_admins_from_db()
            if admin_ids:
                per_page = 10
                pages = [admin_ids[i:i+per_page] for i in range(0, len(admin_ids), per_page)]
                for idx, page in enumerate(pages):
                    admin_list = "
".join([f"🛡️ `{aid}`" for aid in page])
                    send_telegram(chat_id, f"📜 *Danh sách Admin ({idx+1}/{len(pages)}):*
{admin_list}")
            else:
                send_telegram(chat_id, "⚠️ Không có admin nào trong hệ thống.")
...
# ✅ Thêm admin gốc nếu chưa có
def ensure_root_admin():
    ROOT_ADMIN_ID = 6020088518
    if not is_admin(ROOT_ADMIN_ID):
        add_admin_to_db(ROOT_ADMIN_ID)
        print(f"🛡️ Đã thêm admin gốc mặc định: {ROOT_ADMIN_ID}")
